...
function wrapPhoneNumber(phoneNumber: string) {
    if (phoneNumber.length != 10) {
        return "";
    }

    const areaCode = phoneNumber.substring(0,3);
    const officeCode = phoneNumber.substring(3,6);
...
export const PhoneNumberInput: FC = () => {
    ...
    function handlePhoneNumber(event: ChangeEvent<HTMLInputElement>) {
        ...
        let rawPhoneNumber = event.target.value;
        
        const strippedPhoneNumber = rawPhoneNumber.replaceAll(blacklistRegex,"");
        // matches numbers with > 10 digits
        const limitedPhoneNumber = strippedPhoneNumber.replace(/\d{11,}/,strippedPhoneNumber.substring(0,10))
    ...
    if(phoneNumber.length == 10) {
        return (
            ...
            className="m-3 place-self-center rounded-lg border-4 bg-gray-200 border-gray-200 p-0.5 text-black focus:border-gray-400"
            name="phone-number"
        ...
        return <input
          type="tel"
          value={phoneNumber}
          className="m-3 place-self-center rounded-lg border-4 bg-gray-200 border-gray-200 p-0.5 text-black"
          name="phone-number"
          placeholder="XXX-XXX-XXXX"
          pattern="[0-9]{3}[0-9]{3}[0-9]{4}"
          onChange={(event) => {handlePhoneNumber(event)}}
